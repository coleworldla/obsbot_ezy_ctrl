...
- Struct fields the runtime owns and frees (`p_metadata`) are declared as opaque `void *`, never `const char *`: koffi would decode the string and hand back a fresh temporary pointer in `recv_free_video_v2`, which corrupts the heap (found with a ZowieBox that tags every frame with `<Zowietek/>`).
- Video-only cameras (`kind: 'monitor'`) reuse the same receiver; the camera manager just never opens a VISCA socket for them.
